...
interface Props {
  bannerTop: ReactNode
  bannerMidLeft: ReactNode
  bannerMidRight: ReactNode
}

// Intercala los banners mid después de las primeras N motos.
// Si hay <= N resultados, los banners quedan al final como fallback.
const INSERT_AFTER = 8

export default function MotosClient({ bannerTop, bannerMidLeft, bannerMidRight }: Props) {
  ...
  const renderCard = (l: any) => {
    ...
    return (
      <Link key={l.id} href={'/motos/' + (l.slug || l.id)} style={{textDecoration:'none'}}>
  ...
  const midBannerRowStyle: React.CSSProperties = {display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))',gap:'16px',margin:'20px 0'}

  const primeras = motosFiltradas.slice(0, INSERT_AFTER)
  const restantes = motosFiltradas.slice(INSERT_AFTER)
